...
lang TreePPLCompile
  ...
  sem compileIf : TpplCompileContext -> IfExprTpplRecord -> StmtRes
  sem compileIf context = | x ->
    ...
    case (StmtSimple (ts, t), StmtSimple (fs, f)) then
      let t = optionGetOr (withInfo x.info unit_) t in
      let f = optionGetOr (withInfo x.info unit_) f in
  ...
  | IfExprTppl x ->
    match compileIf context x with StmtSimple (ds, e)
    then bindall_ ds (optionGetOr (withInfo x.info unit_) e)
    else errorSingle [x.info] "A conditional expression may not contain 'return' or 'for'."
